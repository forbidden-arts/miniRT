...
/*   Updated: 2023/08/10 08:47:31 by dpalmer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	camera->right = v3d_cross(&camera->direction, &(t_v3d){0, 1, UPSILON});
...
t_ray	create_ray(const t_camera *cam, const t_v2d pixel)
{
	...
	x_norm = (1.0 - (2.0 * (pixel.e[0] + 0.5) / WINDOW_WIDTH))
		* cam->aspect * tan(cam->field_of_view * DEG2RAD);
	y_norm = (1.0 - (2.0 * (pixel.e[1] + 0.5) / WINDOW_HEIGHT))
		* tan(cam->field_of_view * DEG2RAD);
